...
class simple_Delay(app_manager.RyuApp):
    ...
    def _send_echo_request(self):
        ...
        """
        tempdp = self.datapaths.values()
        for datapath in tempdp:
            auxDP = datapath
            # print('send stats request: ', datapath.id)
            parser = auxDP.ofproto_parser
            data =  bytearray(("%.12f" % time.time()).encode())
            echo_req = parser.OFPEchoRequest(auxDP, data)
            auxDP.send_msg(echo_req)
