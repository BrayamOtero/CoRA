...
class Manager(app_manager.RyuApp):
    ...
    def write_values(self):
        ...
        if self.delay is not None:
            for link in self.link_free_bw:
                ...
                self.net_metrics[link] = [round(self.link_free_bw[link],6), round(self.link_used_bw[link],6), round(self.delay.link_delay[link],6), round(self.link_loss[link],6), self.sw_qlen[link], self.sw_qlen[link_rev]]

            file_metrics = '/home/nataliaquino/Tesis/SDNapps_proac/Metrics/'+str(self.monitor.count_monitor)+'_net_metrics.csv'
            with open(file_metrics,'w') as csvfile:
                header_ = ['node1','node2','free_bw','used_bw','delay','pkloss', 'qlen->', '<-qlen']
                ...
                for link, values in sorted(self.net_metrics.items()):
                    ...
                    if tup not in links_in:
                        file.writerow([link[0],link[1],values[0],values[1],values[2],values[3], values[4], values[5]])
            b = time.time()
            ...
            with open('/home/nataliaquino/Tesis/SDNapps_proac/net_info.csv','wb') as csvfile:
    ...
    def calc_loss_path(self,loss_links_path):
        ...
        result_multi = functools.reduce((lambda x, y: x * y), loss_links_path_)
    ...
    def get_k_paths_metrics_dic(self,shortest_paths,bwd_links,delay_links,loss_links):
        ...
        with open('/home/nataliaquino/Tesis/SDNapps_proac/paths_metrics.json','w') as json_file:
    ...
    def get_TM(self, bw_TM):
        ...
        with open('/home/nataliaquino/Tesis/SDNapps_proac/TM.json','w') as json_file:
