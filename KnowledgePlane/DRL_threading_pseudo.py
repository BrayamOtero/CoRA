...
import sys
sys.path.insert(1,'/home/nataliaquino/Tesis/AgentDRL/ddpg_disc_srcdst')
from ddpg_disc_srcdst.ddpg import DDDPGAgent


def get_all_paths(episode_rewards, episode_states_all, episode_duration_all, episodes, k_paths, agent):
    t = time.time()
    env = Environment(step_per_epoch=45)
...
def getKPaths():
    file = '/home/nataliaquino/Tesis/SDNapps_proac/RoutingGeant/DRL/dRSIR/23nodos/k_paths.json'
    with open(file,'r') as json_file:
        k_paths = json.load(json_file)
        return ast.literal_eval(json.dumps(k_paths))

def setPaths(drl_paths):
    with open('/home/nataliaquino/Tesis/SDNapps_proac/drl_paths.json','w') as json_file:
...
if __name__ == "__main__":
    ...
    agent = DDDPGAgent(state_space_size, action_space_size, 1, 0, replay_memory_size, batch_size, 0.003)
    while cont < epoch: 

        a = time.time()
        cont = cont + 1
        time_DRL, episode_rewards, episode_states_all, episode_duration_all = get_all_paths(episode_rewards, episode_states_all, 
                                                                                                        episode_duration_all, episodes, k_paths, agent)
